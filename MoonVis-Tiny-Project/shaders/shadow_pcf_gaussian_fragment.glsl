...
float shadowCoef()
{
 ...
 // Gaussian 3x3 filter
	...
	for(float x = -r; x < r; x += sampleStep)
    {
        ...
        {
			float weight = (r - abs(x)) * (r - abs(y));//权重，中点的权重最高，向四周依次减少
            count += weight; //累加作为权重的总和
            ret += shadow2DArrayOffset(stex, shadow_coord, ivec2(x, y)).x *weight;// 按权重累加邻近坐标的shadow值
            index++;
        }
    }
	ret= ret/ count; //除以权重总和
	*/
...
vec4 getGoochLighting(vec3 vpos, vec3 n)
{
	...
	vec4 u_coolColor = vec4(0.88,0.81,0.49,1.0);
	vec4 u_warmColor = vec4(0.58,0.10,0.76,1.0);
    diffuse= mix(u_coolColor*0.9, u_warmColor*0.9,blendval);
   ...
   const float Kd=0.8;
   const float Ks=0.9;
	vec4 fColor = Ka* ambient +Kd* diffuse + Ks*specular;
...
void main()
{
	...
	//vec4 fcolor = getGoochLighting(vPos.xyz, normalize(normal+normal_bias)) * color_tex;
