...
float map(vec3 p) {
    float d = 10000.0;
    float s = 1.0;
    p.xz *= rotate(time * 0.39*0.5);
    for (int i = 0; i < 4; i++) {
        p.yz *= rotate(time * 0.034*0.5);
        p = abs(p);
        p -= 15.0;
        p.xz *= rotate(time * .194*0.5);
        p.xy *= rotate(time * .132*0.5);
...
void main(void) {
    ...
    vec3 ro = vec3( 0.0, 0.0, -50.0 + (fbm(time * 0.03*0.5) * 2.0 - 1.0) * 25.0);
    vec3 ta = vec3(0.0);
